...
export function PromptQueue({ queue, paused, busy, onCancel, onResume }: {
  ...
  paused: boolean;
  busy: boolean;
  onCancel: (item: QueuedPrompt) => void;
  onResume: () => void;
}) {
  ...
  return <section className={`prompt-queue ${paused ? "is-paused" : ""}`} aria-label="待处理消息队列">
    ...
    <div>{queue.map((item, index) => <article key={item.id}><span>{index + 1}</span><p>{item.message || "请查看附加图片"}{item.imageCount > 0 && <small>{item.imageCount} 张图片</small>}</p><button type="button" className="prompt-queue-cancel" disabled={busy} onClick={() => onCancel(item)} aria-label="撤销队列消息">撤销</button></article>)}</div>
  </section>;
}
