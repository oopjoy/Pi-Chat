...
import type { BootstrapData, CustomModelInput, ModelInfo, PiState, PluginResource, SkillResource } from "../../shared/types";
import { api } from "../api";
...
type SettingsTab = "appearance" | "skills" | "plugins";
...
export function ManagementPanel({ section, appearance, models, state, busy, onClose, onAppearance, onModel, onReloaded }: {
  ...
  useEffect(() => {
    if (section === "settings") setSettingsTab("appearance");
    setFilter("");
    setResourceError("");
  }, [section]);
  ...
  useEffect(() => {
    setResourceError("");
    if (section !== "settings" || (settingsTab !== "skills" && settingsTab !== "plugins")) return;
    setLoading(true);
    const load = settingsTab === "skills" ? api.skills() : api.plugins();
    load.then((result) => {
      if (settingsTab === "skills") setSkills(result.resources as SkillResource[]);
      else setPlugins(result.resources as PluginResource[]);
    }).catch((error) => setResourceError(error instanceof Error ? error.message : String(error))).finally(() => setLoading(false));
  }, [section, settingsTab]);
  if (!section) return null;

  const runResource = async <T extends SkillResource | PluginResource>(operation: () => Promise<{ resources: T[] }>, apply: (items: T[]) => void) => {
  ...
  const applyPlugins = (items: PluginResource[]) => setPlugins(items);

  return (
    <div className="panel-backdrop" role="presentation" onMouseDown={(event) => event.target === event.currentTarget && onClose()}>
      <section className={`management-panel ${section === "settings" ? "panel-settings" : ""}`} role="dialog" aria-modal="true" aria-labelledby="management-title">
        <header className="management-head">
          <div>
            <span className="management-kicker">Pi Chat</span>
            <h2 id="management-title">{section === "settings" ? "设置" : "Models"}</h2>
          </div>
          ...
          <div className="settings-workspace">
            ...
            <div className="settings-content">
              ...
              {settingsTab === "skills" && <SettingsResourceList
                title="Skills" loading={loading} error={resourceError} resources={skills} busy={resourceBusy}
                pathFor={(item) => item.pathLabel}
                onToggle={(item) => void runResource(() => api.toggleSkill(item.id, !item.enabled), applySkills)}
              />}
              {settingsTab === "plugins" && <SettingsResourceList
                title="Plugins" loading={loading} error={resourceError} resources={plugins} busy={resourceBusy}
                pathFor={(item) => item.installedPath || item.source}
                onToggle={(item) => void runResource(() => api.togglePlugin(item.id, !item.enabled), applyPlugins)}
              />}
            </div>
          </div>
        )}

        {section === "models" && <ModelsPanel models={models} state={state} busy={busy} filter={filter} onFilter={setFilter} onModel={onModel} onReloaded={onReloaded} />}
      </section>
    </div>
  );
}
...
function ModelsPanel({ models, state, busy, filter, onFilter, onModel, onReloaded }: {
  ...
  return <div className="panel-body models-panel">
    <div className="models-toolbar"><PanelIntro title="可用模型" description="切换模型，或管理 ~/.pi/agent/models.json 中的自定义模型。" count={models.length} /><button type="button" className="model-add-button" disabled={busy || modelBusy} onClick={() => { setAdding((value) => !value); setModelError(""); }}>{adding ? "取消" : "＋ Add"}</button></div>
    ...
    {adding && <div className="model-form">
      ...
      <button type="button" className="model-save-button" disabled={busy || modelBusy || !form.provider.trim() || !form.id.trim()} onClick={() => void add()}>{modelBusy ? "保存中…" : "保存模型"}</button>
    </div>}
    ...
    <div className="model-grid">{visible.map((model) => {
      ...
      return <article className={`model-card ${active ? "is-active" : ""}`} key={`${model.provider}/${model.id}`}>
        ...
        {model.custom && <div className="model-card-actions"><button type="button" className="is-danger" disabled={busy || modelBusy || active} title={active ? "请先切换到其他模型" : "从 models.json 删除"} onClick={() => void remove(model)}>Remove</button></div>}
      </article>;
    })}</div>
  </div>;
}
...
function PanelIntro({ title, description, count }: { title: string; description: string; count: number }) {
  return <div className="panel-intro"><div><h3>{title}</h3><p>{description}</p></div><span className="count-badge">{count}</span></div>;
}

function SettingsResourceList<T extends { id: string; name: string; enabled: boolean }>({ title, loading, error, resources, busy, pathFor, onToggle }: {
  title: string;
  loading: boolean;
  error: string;
  resources: T[];
  ...
  return <div className="settings-resource-panel">
    <PanelIntro title={title} description="只显示本地已发现资源；切换后 Pi 会自动重新加载配置。" count={resources.length} />
    ...
    <div className="settings-resource-list">
      {loading ? <p className="resource-loading">正在扫描…</p> : resources.map((item) => <article key={item.id} className="settings-resource-row">
        ...
        <Toggle enabled={item.enabled} busy={busy} onClick={() => onToggle(item)} />
      </article>)}
...
function AppearancePanel({ value, onChange }: { value: AppearancePreferences; onChange: (value: AppearancePreferences) => void }) {
  ...
  return <div className="panel-body appearance-panel"><PanelIntro title="外观与阅读" description="修改会即时生效，并保存在当前浏览器中。" count={5} /><SettingRow title="主题" description="跟随系统或固定明暗主题"><select value={value.theme} onChange={(event) => update("theme", event.target.value as ThemePreference)}><option value="system">跟随系统</option><option value="light">浅色</option><option value="dark">深色</option></select></SettingRow><SettingRow title="字体" description="控制消息和界面的主要字体"><select value={value.font} onChange={(event) => update("font", event.target.value as FontPreference)}><option value="system">系统字体</option><option value="serif">衬线阅读字体</option><option value="mono">等宽字体</option></select></SettingRow><RangeSetting title="字号" value={value.fontSize} minimum={13} maximum={22} step={1} suffix="px" onChange={(next) => update("fontSize", next)} /><RangeSetting title="行间距" value={value.lineHeight} minimum={1.35} maximum={2.2} step={0.05} suffix="" onChange={(next) => update("lineHeight", next)} /><RangeSetting title="对话宽度" value={value.chatWidth} minimum={700} maximum={1500} step={50} suffix="px" onChange={(next) => update("chatWidth", next)} /><div className="appearance-preview"><span>预览</span><p>Pi Chat 会按照当前字号、行距和宽度显示 Markdown 与数学公式。</p></div></div>;
}
