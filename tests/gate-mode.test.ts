...
test("gate mode parser recognizes aliases and runtime status notifications", () => {
  ...
  assert.equal(gateModeFromCommand("/gate next"), null);
  assert.equal(gateModeFromCommand("/gate allow"), "open");
  assert.equal(gateModeFromCommand("/gate status"), null);
  assert.equal(gateModeFromCommand("/gate open extra"), null);
  ...
  assert.equal(gateModeFromNotice("Gate mode: once\nThe next write will be allowed."), null);
  assert.equal(gateModeFromNotice("Gate will allow the next write/edit/destructive bash call."), null);
  assert.equal(gateModeFromNotice("Gate one-shot allow used for edit: app.ts"), null);
});
