...
- 历史会话列表、切换和新建；服务启动后按固定预算顺序预热最近 3 个历史会话，最多保留 3 个空闲 Secondary Runtime。点击预算外冷历史只读取 JSONL，不创建第 4 个 Runtime；正在显示的历史仍可在空闲时被容量回收并自动退回 view-only，下次实际操作再按需恢复
...
- 侧栏提供独立刷新和“完整重启 Pi Chat 并应用更新”：应用级 Lifecycle Barrier 会在构建前同步阻止所有新写操作，构建完成后再次核验全部 Runtime、队列和确认状态，再执行服务切换；维护期间历史、健康检查和只读 API 保持可用。设置中的“关闭 Pi Chat”同样先执行全局 Busy 检查，再关闭全部窗口、服务和托管 RPC
