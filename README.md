...
- 同一 Session 可在多个窗口观察，但同一时刻仅一个浏览器窗口可发送、停止、修改模型/Thinking、处理 Gate 或改队列；其他窗口须显式“接管控制”后才可操作
- 文件权限 Gate：作为 Pi Chat 内置安全功能呈现；顶栏可切换“严格 / 仅一次 / 放行”，随应用自动安装、校验和修复的极小 Pi 工具执行适配器仍会在真实工具执行前拦截写入、编辑和危险 Bash
- 侧栏提供独立的刷新会话列表和“完整重启 Pi Chat 并应用更新”按钮：会结束 Pi Chat 服务及其管理的全部 Pi RPC worker，重新构建当前 Pi Chat 工作目录，再启动全新的服务与会话 worker；不会删除 Session JSONL 或聊天记录，也不会影响非 Pi Chat 启动的 Pi 进程；生成、排队或等待权限确认时禁止操作。设置面板左下角的“关闭 Pi Chat”则只结束服务与其管理的全部 worker，聊天记录和设置会保留，可用桌面启动器重新打开
...
Windows 上提供两个桌面启动器：**Pi Chat（网页）** 会启动服务后以普通 URL 打开；**Pi Chat（Edge PWA）** 会启动服务后打开已安装的 Edge PWA。两者都调用 `pi-chat-launch.cmd`，服务未运行时会先构建当前工作目录。也可以双击兼容入口 `start-pi-chat.cmd`，它等同于网页启动器。
...
安装后会以独立 Pi Chat 窗口启动，不显示地址栏、标签页、书签栏或浏览器导航；普通浏览器访问仍然可用。该模式不使用 Service Worker，避免本地更新后被旧前端缓存遮挡。关闭浏览器窗口不会停止后台 Pi Chat 服务；请使用侧栏的“完整重启 Pi Chat 并应用更新”，或结束服务后通过 `start-pi-chat.cmd` / `pi-chat-launch.cmd` 重新启动。源码工作目录的启动器会先构建本地改动；Windows Release ZIP 已内置干净的编译产物，不需要 npm、源码或构建工具即可启动。
