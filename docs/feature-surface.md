...
| Files / Changes inspector | Resizable right sidebar with lazy Workspace file tree, bounded read-only text preview, and existing Edit-derived Changes view | `GET /api/sessions/:id/workspace/files`, `GET /api/sessions/:id/workspace/file` | Session-addressed cold reads never start a Runtime. Hidden/generated/dependency/sensitive/symlink/out-of-root paths and binary previews fail closed. Changes remains an Edit projection, not repository-wide Git status. |
