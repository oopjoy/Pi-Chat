...
| Failure reason visibility | Conversation body of the affected Session | No new API: Pi-persisted failed attempts plus a browser-local bounded entry | A failed model attempt that Pi persisted renders as a bounded card in the transcript, including the provider route Pi recorded for it (`provider · model · api`), so a provider that differs from the composer selection is visible instead of inferred. A Runtime or upstream failure that produced no assistant message keeps a browser-local entry in that Session body rather than only the five-second toast; entries never enter Pi JSONL or a provider payload, are capped per Session and per app, and are suppressed only when the same failure category is already persisted nearby. The composer keeps its own persistent Runtime status surface; input-validation rejections and user aborts stay transient. |
| Attachments | Composer attachment menu, paste, drag and drop | `POST /api/local-files/pick` | Local paths and image payloads only; no upload service. Image prompts allow up to 10 images, 8 MB each and 40 MB decoded total. |
...
| Files / Changes inspector | Resizable right sidebar whose Files tab lists the selected Session's most recent successful Edit/Write targets, provides a vertically resizable list/preview split and horizontally scrollable bounded code preview, and retains the existing Edit-derived Changes view | `GET /api/sessions/:id/workspace/files`, `GET /api/sessions/:id/workspace/file` | Recency follows transcript tool-result order, not filesystem mtime. Preview is authorized only for the projected recent-file set, and cold reads never start a Runtime. Hidden/generated/dependency/sensitive/symlink/out-of-root paths and binary previews fail closed. Changes is not repository-wide Git status. |
| Runtime preparation | Send, Compact, model/thinking change, explicit internal warm path | `POST /api/sessions/:id/warm`, `POST /api/sessions/:id/activate` | Capability upgrade for real work, not a sidebar resource-management feature. |
| Session control | Multi-window shared write with observer banner | presence and viewing leases | Every live window submits into one Session-scoped FIFO consumed by the single live Agent; presence/viewing are observation-only projections. Rename/Delete keep the exclusive destructive lock. |
...
| Session clone and User-message fork | Session action menu with an explanatory Clone confirmation; persisted text User-message action with a Fork-boundary preview and confirmation; destination provenance banner with source navigation | `POST /api/sessions/:id/clone`, `POST /api/sessions/:id/fork` plus Pi Chat's atomic `pi-chat-session-relations.json` sidecar | Neither operation starts from its first click. Fork explains that history before the selected User message is copied and the selected text becomes an unsent draft. Both create a separate cold Session and never mutate Pi JSONL. Fork provenance survives restart outside model context; a deleted source fails closed as unavailable. |
