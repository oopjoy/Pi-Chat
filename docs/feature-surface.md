...
| Background Subagent visibility | Compact current-Session TopBar count and accessible read-only popover | `GET /api/sessions/:id/background-subagents` | Exact parent JSONL path only; child Sessions never enter Sidebar. Safe aliases/status/age/counts only, with no transcript navigation or spawn/steer/resume/interrupt/stop controls. Main Queue/Steer semantics are unchanged. |
