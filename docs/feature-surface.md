...
| Session clone and User-message fork | Session action menu with an explanatory Clone confirmation; persisted text User-message action | `POST /api/sessions/:id/clone`, `POST /api/sessions/:id/fork` | Clone never starts from the menu click alone: the user must confirm or cancel after being told that the source is unchanged and the destination becomes independent. Both operations create a separate cold Session and never mutate the source JSONL. |
