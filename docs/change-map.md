...
| 右侧 Files / Changes 只读检查器 | `workspace-files.ts`、`routes/workspace-read.ts`、`EditToolDiff.tsx`、`App.tsx` | 服务端仅投影 Session Workspace 内有界目录层级和文本前缀；浏览器 Files tab 拥有可丢弃树/预览状态；Changes 继续复用 Edit 参数派生 Diff，不获得 Git 或文件写 authority | `workspace-files.test.ts`、`workspace-inspector.test.ts`、`conversation-process.test.ts`、`e2e/ui.spec.ts` |
