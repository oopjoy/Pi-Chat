...
| Pi launch plan / RPC Bundle / startup phases | `pi-runtime-bundle.ts`、`rpc-client.ts`、`scripts/build-pi-runtime.mjs`、`resources/runtime/pi-chat-startup-probe.mjs` | 服务启动时冻结一个 Primary/Secondary 共用 launch plan；每个 Session 仍拥有独立 child。Bundle 只在 Pi version、critical source hash、全部 output hash 精确匹配时启用；诊断 span 只观察 spawn/pre-entry/get_state/stdout，不参与 readiness 或 writer authority | `pi-runtime-bundle.test.ts`、`pi-runtime-build.test.ts`、`rpc-client.test.ts`、`incident-diagnostics.test.ts` |
