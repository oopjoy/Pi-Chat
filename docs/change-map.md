...
| Pi launch plan / RPC Bundle / startup phases | `pi-runtime-bundle.ts`、`rpc-client.ts`、`scripts/build-pi-runtime.mjs`、`resources/runtime/pi-chat-startup-probe.mjs` | 服务启动时冻结一个 Primary/Secondary 共用 launch plan；每个 Session 仍拥有独立 child。Bundle 只在 Pi version、全部 Bundle input hash、全部 output hash 和配方精确匹配时启用；诊断 span 记录 Bundle/direct backend、entry evaluation、Bundle entry、Extension import/factory、get_state/stdout 等脱敏阶段，不参与 readiness 或 writer authority | `pi-runtime-bundle.test.ts`、`pi-runtime-build.test.ts`、`rpc-client.test.ts`、`incident-diagnostics.test.ts` |
...
| Gate / Extension UI / 模型澄清问卷 | `runtime-event-transition.ts`、`app.ts` pending request/response、`ExtensionDialog.tsx`、`AskQuestionnaireDialog.tsx`、`lib/ask-questionnaire.ts` | Pi Extension 工具与 RPC request ID 保持执行权；服务端持有 Session/control/first-response 路由；Ask 组件只投影已进入工具调用的 bounded questions，并在最终提交后将完整草稿映射回 Package 现有的有序标量请求 | `ask-questionnaire.test.ts`、`ask-questionnaire-dialog.test.ts`、`extension-dialog.test.ts`、`server/session-read-sse-extension.test.ts`、`web/pane-prompt-authority.test.ts`、`web/pane-recovery-authority.test.ts` |
| SSE 节流、顺序和背压 | `sse-hub.ts` | `SseHub` 只持有 transport client/pending frame | `sse-hub.test.ts`、`sse-transport-recovery.test.ts` |
| 消息投影身份 | `live-message-identity.ts`、`session-index.ts`、`streaming-assistant.ts` | 服务端分配 Session-scoped live lifecycle ID；`SessionIndex` 从 active JSONL entry 派生 persisted ID；浏览器优先使用结构身份并仅为旧投影保留兼容相关性 | `live-message-identity.test.ts`、`pi-events.test.ts`、`session-index.test.ts`、`conversation-process.test.ts` |
| Workspace 默认值与 draft picker | `App.tsx`、`workspace-state.ts`、`app.ts` | 服务端 workspace snapshot；浏览器各 picker token | `draft-workspace.test.ts`、`workspace-state.test.ts`、`web/app-workspace-replacement.test.ts`、`server/workspace-resource-lifecycle.test.ts` |
| 右侧 Files / Changes 只读检查器 | `workspace-files.ts`、`routes/workspace-read.ts`、`EditToolDiff.tsx`、`App.tsx` | 服务端从 Session transcript 成功 Edit/Write 结果投影最近修改文件，并只允许预览该集合中的有界文本；浏览器拥有可丢弃列表/预览状态、可纵向调整的上下分区和支持横向滚动的代码预览；Changes 继续复用 Edit 参数派生 Diff，不获得 Git 或文件写 authority | `workspace-files.test.ts`、`workspace-inspector.test.ts`、`conversation-process.test.ts`、`e2e/ui.spec.ts` |
| Clone / Fork to new Session | `ChatMessage.tsx` persisted User action、`SessionSidebar.tsx` Clone menu、`SessionDialog.tsx` Clone/Fork confirmations、`SessionForkBanner.tsx` provenance link、`session-relations.ts` atomic sidecar、`App.tsx` Pane/selection/draft authority、`app.ts` copy orchestration、`runtime-pool.ts` source recovery、`session-index.ts` target validation | Clone 与 Fork 都必须先解释语义并由用户确认或取消；Fork 弹窗明确复制边界和未发送草稿行为。服务端只接受当前 active branch 的 text-only persisted User；同一 source RPC 在 Session-scoped admission 内临时切到 destination，验证后必须恢复 immutable source binding，禁止第二 writer；destination 仅作为 cold Session 发布；来源关系不进入 Pi JSONL 或模型上下文，重启后仍可返回原对话，来源删除则 fail closed | `session-index.test.ts`、`session-management.test.ts`、`session-relations.test.ts`、`session-fork-banner.test.ts`、`chat-message.test.ts`、`web/session-inventory-mutations.test.ts` |
...
| 等待中的本地 turn 只在 Queue，dispatch 后才进入 transcript | Prompt/local-turn contract | `web/queue-dispatch-reconciliation.test.ts`、`local-user-turn`、`prompt-scheduler` |
