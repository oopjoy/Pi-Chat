...
| Clone / Fork to new Session | `ChatMessage.tsx` persisted User action、`SessionSidebar.tsx` Clone menu、`SessionDialog.tsx` Clone confirmation、`App.tsx` selection/draft authority、`app.ts` copy orchestration、`runtime-pool.ts` source recovery、`session-index.ts` target validation | Session 菜单 Clone 必须先解释独立复制语义并由用户确认或取消；服务端只接受当前 active branch 的 text-only persisted User；同一 source RPC 在 Session-scoped admission 内临时切到 destination，验证后必须恢复 immutable source binding，禁止第二 writer；destination 仅作为 cold Session 发布；浏览器只有在 navigation epoch 未变化时自动打开，Fork 文本按 Composer partition/revision 恢复 | `session-index.test.ts`、`session-management.test.ts`、`chat-message.test.ts`、`web/session-inventory-mutations.test.ts` |
