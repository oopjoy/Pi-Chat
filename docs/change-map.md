...
| Gate / Extension UI / 模型澄清对话框 | `runtime-event-transition.ts`、`app.ts` pending request/response、`ExtensionDialog.tsx` | Pi Extension 工具与 RPC request ID 保持执行权；服务端持有 Session/control/first-response 路由；组件只渲染并返回原值 | `extension-dialog.test.ts`、`server/session-read-sse-extension.test.ts`、`web/queue-steer-extension.test.ts`、`web/pane-authority.test.ts` |
